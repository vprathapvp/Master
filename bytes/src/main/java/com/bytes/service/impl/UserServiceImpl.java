...
public class UserServiceImpl implements UserService {
	...
	// subscribe
	@Override
	public void subscribeToChannel(String channelEmail) {
		...
		User channel = userRepository.findByEmail(channelEmail);

		if (user != null && channel != null) {
			List<String> subscribedChannels = user.getSubscribedChannels();
			if (!subscribedChannels.contains(channelEmail)) {
				subscribedChannels.add(channelEmail);
				userRepository.save(user);
			}
		}
	}
	...
	// unsubscibe
	@Override
	public void unsubscribeFromChannel(String channelEmail) {
		String userEmail = getAuthenticatedUserEmail();
		if (userEmail == null) {
			return;
		}
		...
		if (user != null && channel != null) {
			...
			if (subscribedChannels.contains(channelEmail)) {
				subscribedChannels.remove(channelEmail);
				userRepository.save(user);
			}
		}
	}
